---
title: svchost.exe
description: Windows uses svchost.exe to run system services that are shared among other processes. Services are grouped into many individual svchost.exe processes to improve reliability in case one service crashes. The system services running in SvcHost are DLL (dynamic link library) files.
